...
def generate_propeller(
    ...
    # Old style incorrectly interpolated from 0 to R
    # New style interpolates from radius_hub to R
...
def propellor_calcs(config_file="propeller.ini", mass=100, TW_Constraint=1.5):
    """
...
# If rpm is not found, raise an error
    ...
    # Required thrust based on constraint
    # Could move this outside the propellor module and keep it general.
    T_required_total = TW_Constraint * mass * 9.81
    T_required_motor = T_required_total/4

    if (T < T_required_motor):
        raise Warning("Violating thrust to weight constraint")

    return T, Q, P #, rpm is not a dependent variable so do not return it.

#########################################################################################
#Adam commented out propellor_calcs2 because he does not think it makes sense to do it that way.
# Just check if the constraint is violated or not. That's it. Do not attempt to force it. That's for optimization not individual runs.
